...
function parseAuthority(value) {
  ...
  if (authority.startsWith('[')) {
    ...
    return { host, port: cleanPort(suffix.startsWith(':') ? Number(suffix.slice(1)) : 443) };
  }
  const separator = authority.lastIndexOf(':');
  if (separator <= 0) return { host: cleanHost(authority), port: 443 };
  return { host: cleanHost(authority.slice(0, separator)), port: cleanPort(authority.slice(separator + 1)) };
}

function socksOptions(proxy) {
  const type = String(proxy?.type || '').toLowerCase() === 'socks4' ? 4 : 5;
  const options = { host: cleanHost(proxy?.host), port: cleanPort(proxy?.port), type };
...
export async function startSocksHttpBridge(proxy) {
  ...
  const server = http.createServer((_request, response) => {
    ...
    response.end('userFLEX proxy bridge accepts HTTPS CONNECT only.');
  });
