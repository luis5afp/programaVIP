...
assert.equal(compareVersions('0.3.36', '0.3.36'), 0);
assert.equal(compareVersions('0.3.36', '0.3.35'), 1);
assert.equal(compareVersions('0.3.35', '0.3.36'), -1);
...
assert.equal(versionAtLeast('0.3.35', MIN_SESSION_MANAGER_VERSION), true);
assert.equal(versionAtLeast('0.3.34', MIN_SESSION_MANAGER_VERSION), false);
...
assert.equal(sessionPackage.version, '0.3.35');
assert.equal(MIN_USERFLOW_VERSION, '0.3.36');
assert.equal(MIN_SESSION_MANAGER_VERSION, '0.3.35');
assert.notEqual(clientPackage.version, sessionPackage.version, 'desktop applications must be independently versioned');
