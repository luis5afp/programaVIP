...
assert.equal(versionAtLeast('0.3.38', MIN_USERFLOW_VERSION), true);
assert.equal(versionAtLeast('0.3.37', MIN_USERFLOW_VERSION), false);
...
assert.equal(clientPackage.version, '0.3.38');
assert.equal(sessionPackage.version, '0.3.43');
assert.equal(MIN_USERFLOW_VERSION, '0.3.38');
